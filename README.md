...
### Representation
Basemap: A U.S. basemap displaying state boundaries and geographic context. National parks are represented as point features positioned accurately across the country.

National Park Features: Each national park is represented as an interactive point symbol. Symbol color or size encodes an overall climate risk index, allowing users to quickly identify high-risk parks.

Climate Hazard Layers: Multiple climate hazard datasets will be represented, including: wildfire risk, temperature change, drought index. These variables will be visualized using choropleth-style color gradients or proportional symbols, allowing users to interpret spatial patterns and intensity.

Temporal Representation: A temporal dimension will be incorporated using a time slider, allowing users to view historical data or projected future scenarios.

Attribute Panel: When a user selects a park, a side panel will display: park name and location, hazard-specific values, descriptive information and images of the park.

Comparison Panel: A coordinated table or chart will display selected parks and their attributes, enabling side-by-side comparison across multiple variables.


### Interaction
Zoom & Pan: Users can pan and zoom to explore different national parks and scales.

Click Selection: Users can click on a park to: identify the park, view detailed attributes, view images of the park, highlight the park across coordinated views.

Hover Interaction: Users can hover over park symbols to preview summary information such as name and key risk indicators.

Layer Toggle: Users can turn on/off different hazard layers, allowing exploration of individual or combined risks.

Filter: Users can filter parks based on: hazard type, risk thresholds, geographic region.

Time Slider: Users can use a time slider to explore how climate risks change over time, supporting temporal analysis.

Search: Users can search for a specific park by name, which will highlight and center the map on that location.

Comparison Selection: Users can select multiple parks to compare attributes side-by-side in a table or chart view.

Linked Highlighting: Selecting a park will highlight multiple views (map, table, charts).


## Wireframe mock-ups of map solution
